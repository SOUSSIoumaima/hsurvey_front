...
const EditOrganizationModal = ({ open, organization, onClose, onSuccess }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    setError('');
    try {
      await apiService.updateOrganization(organization.id, form);
      onSuccess && onSuccess();
      onClose();
    } catch (err) {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm animate-fade-in">
      <div className="bg-white rounded-2xl shadow-2xl p-8 w-full max-w-md relative transform transition-all duration-300 scale-100 animate-slide-up border border-gray-100">
        ...
        <div className="flex items-center gap-4 mb-8">
          ...
          <div>
            <h3 className="text-2xl font-bold text-gray-800">Edit Organization</h3>
            <p className="text-gray-600 mt-1">
              Update organization information for <span className="font-semibold text-gray-800">{organization.name}</span>
            </p>
          </div>
        </div>
